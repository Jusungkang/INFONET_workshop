...
for zzz in range(Avg_times):
    ...
    SNR_result = np.zeros((3, len(SNR_range)))
    ...
    for k in range(len(SNR_range)):
        ...
        for epoch in range(training_epoch):
            ...
            print(
                '[Avg : {:>=2}][SNR : {:>=4}] (Epoch: {:>3}) Train cost = {:>.6} with learning rate = {:>.4}'.format(
                    zzz , SNR, epoch + 1, avg_cost, optimizer.param_groups[0]['lr']))

            ##
            ## Training 결과에 따라, 가장 낮은 Training cost 를 보이는 Model 을 저장하는 내용.
            ##

            if (np.min(train_cost) == avg_cost) == True:
                # save
                check_point = epoch + 1
                savePath = "./Models/RF_fingerprinting/[" + str(zzz) + "][" + model_version + "]Classifier_" + str(SNR) + ".pth"
        ...
        np.savetxt('./Models/RF_fingerprinting/[' + str(zzz) + '][' + model_version + ']Training_result.csv', SNR_result)
